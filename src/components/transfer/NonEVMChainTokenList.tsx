...
const NonEVMChainTokenList: FC<IProps> = ({
  onSelectToken,
  visible,
  onCancel,
}) => {
  ...
  const { fromChain, toChain, flowTokenPathConfigs } = useAppSelector(
  ...
  const [tokenListWithBalance, setTokenListWithBalance] = useState(
    transferSupportedTokensResult.supportTokenList
  );
  ...
  const updateTokenListBalance = useMemo(
    () =>
      debounce(async () => {
        ...
        const fromChainNonEVMMode = getNonEVMMode(
          transferSupportedTokensResult.fromChainId
        );

        switch (fromChainNonEVMMode) {
          case NonEVMMode.flowMainnet:
          case NonEVMMode.flowTest: {
            await getFlowTokenListBalance(
              nonEVMAddress,
              transferSupportedTokensResult
            );
            break;
          ...
          case NonEVMMode.terraTest: {
            await getTerraTokenListBalance(
              nonEVMAddress,
              transferSupportedTokensResult
            );
            break;
          }
          case NonEVMMode.defichainMainnet:
            break;
          case NonEVMMode.off: {
            break;
          }
  ...
  const getFlowTokenListBalance = async (
    flowAddress: string,
    ...
    supportedTokenList.forEach((tokenInfo) => {
      ...
      const getFlowBalancePromise = checkTokenBalanceForFlowAccount(
        flowAddress,
        flowTokenPath?.BalancePath ?? ""
      )
        .then((balance) => {
          return {
            ...tokenInfo,
  ...
  const getTerraTokenListBalance = async (
    terraAddress: string,
    result: SupportTokenListResult
  ) => {
    const supportedTokenList = result.supportTokenList;
    ...
    terraGeneralTokens.forEach((tokenInfo) => {
      const getTerraGeneralTokenBalancePromise = terraGeneralTokenBalance(
        terraAddress,
    ...
    await terraNativeBalances(terraAddress).then((coins) => {
      terraNativeTokens.forEach((tokenInfo) => {
        ...
        if (coin) {
          const balance = formatDecimalPart(
            `${Number(coin.amount) / 1000000}`,
            6,
            "floor",
            true
          );
  ...
  const addBalancePlaceHolderForTokenList = (
    supportedTokenList: TokenInfo[]
  ) => {
  ...
  useEffect(() => {
    ...
    sortedTokenList.sort((a, b) =>
      Number(a.balance?.split(",").join("")) >
      Number(b.balance?.split(",").join(""))
        ? -1
        : 1
    );
  ...
  useEffect(() => {
    ...
    const list = filterTokenList?.filter((item) => {
      ...
      const addressFeatch =
        item.token.address.toString().toLowerCase().indexOf(searchText) > -1;
      const symbolFeatch =
        item.token.symbol.toString().toLowerCase().indexOf(searchText) > -1;
  ...
  const sortTokenList = (list: TokenInfo[]) => {
    ...
    sortExclusiveTokenSymbols.forEach((symbol) => {
      const targetToken = highPriorityTokenList.find(
        (item) =>
          item.token.symbol === symbol || item.token.display_symbol === symbol
      );
    ...
    const result = sortedHighPriorityTokenList.concat(
      sortedNormalTokenList
    ) as Array<TokenInfo>;
    return result;
  };
      ...
      <Spin spinning={loading} wrapperClassName="tokenSpin">
        <div className={classes.card}>
          ...
          <div className={classes.moreOptionNote}>
            ...
            >
              Below is the supported token list from {fromChain?.name} to{" "}
              {toChain?.name}.{" "}
              <span style={{ color: "#8F9BB3", fontSize: 14, fontWeight: 700 }}>
                More tokens
              </span>{" "}
              can be found if you select other chains.
            </span>
          </div>
