...
const TokenList: FC<IProps> = ({ onSelectToken, visible, onCancel }) => {
        ...
        switch (fromChainNonEVMMode) {
          ...
          case NonEVMMode.off: {
            if (chainId !== transferSupportedTokensResult.fromChainId) {
              ...
              addBalancePlaceHolderForTokenList(
                transferSupportedTokensResult.supportTokenList
              );
  ...
  const isNativeToken = (
    ...
    if (
      tokenDisplaySymbol === "ETH" &&
      ethSupportedChainIds.includes(targetChainId)
    ) {
      return true;
    }
    ...
    if (targetChainId === 560000 && tokenSymbol === "DFI") {
      return true;
    }
    ...
    if (
      (targetChainId === 137 || targetChainId === 80001) &&
      tokenSymbol === "MATIC"
    ) {
      return true;
    }
