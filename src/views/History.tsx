...
const useStyles = createUseStyles<string, { isMobile: boolean }, Theme>(
  (theme: Theme) => ({
    ...
    historyBody: {
      ...
      boxShadow:
        "0px 4px 17px rgba(51, 102, 255, 0.1), 0px 8px 10px rgba(51, 102, 255, 0.1)",
    },
    mobileHistoryBody: {
      width: "100%",
      height: "100%",
...
const History = (props: IProps): JSX.Element => {
  ...
  const [historykey, setHistorykey] = useState(
    featureSupported === FeatureSupported.NFT
      ? "nft_history"
      : "transfer_history"
  );
  ...
  const getTransactionOnchainQueryPromiseList = (
    provider,
    chainId,
  ...
  const getHistoryList = async (next_page_token) => {
    ...
    if (flowAddress.length > 0) {
      const flowEVMCompatibleAddress =
    ...
    if (terraAddress.length > 0) {
      const terraEVMCompatibleAddress =
        await convertNonEVMAddressToEVMCompatible(
          terraAddress,
    ...
    if (res) {
      ...
      const allItemTxQueryPromise = getTransactionOnchainQueryPromiseList(
        provider,
        chainId,
        localHistoryList
      );
  ...
  useEffect(() => {
    if (mergedHistoryList && mergedHistoryList.length > 0) {
      const renewPageToken =
        mergedHistoryList[mergedHistoryList.length - 1].ts.toString();
  ...
  const getNFTOnChainQueryPromiseList = (
    provider,
    chainId,
    localNFTHistoryList: NFTHistory[]
  ) => {
    // eslint-disable-next-line
    const promiseList: Array<Promise<any>> = [];
  ...
  const getNFTHistoryList = async (nextPage, isAutoRefresh = false) => {
    if (
      featureSupported !== FeatureSupported.BOTH &&
      featureSupported !== FeatureSupported.NFT
    ) {
      return;
    }
    ...
    const res = await nftHistory(address, {
    ...
    if (res && res.history) {
      ...
      let localNftList;
      const localNftListStr = localStorage.getItem(
        storageConstants.KEY_NFT_HISTORY_LIST_JSON
      );
      if (localNftListStr) {
        localNftList = JSON.parse(localNftListStr)[address] as NFTHistory[];
      }
      const promiseList = getNFTOnChainQueryPromiseList(
        provider,
        chainId,
        localNftList
      );
  ...
  const debouncedMessageNumberChangeHandler = useCallback(
    debounce(refreshTabMessageCallback, 300),
    []
  );
  ...
  const clearLpLocalData = (item) => {
    ...
    if (localLpListStr) {
      ...
      localLpList?.map(async (localItem, i) => {
        if (
          (Number(item.nonce) === Number(localItem.nonce) &&
            item.type === LPType.LP_TYPE_ADD) ||
          (Number(item.seq_num) === Number(localItem.seq_num) &&
            item.type === LPType.LP_TYPE_REMOVE)
        ) {
  ...
  const clearHistoryLocalData = (item) => {
    const transferListStr = localStorage.getItem(
  ...
  const tipsStatus = (item, peggedMode) => {
    ...
    const showResult =
      nowDate - Number(item.updateTime || item.ts) <=
      tooltipShowTime * 60 * 1000;
    switch (item.status) {
        ...
        lab = (
          <Tooltip
            ...
            title={
              showResult ? (
                <span>
                  Your transfer is being confirmed on{" "}
                  {item.src_send_info.chain.name}. Please allow{" "}
                  {item.src_send_info.chain.block_delay} block confirmations (a
                  few minutes) for your transfer request to be confirmed.
                </span>
              ) : (
                <div>
                  It seems that your transaction has been stuck for more than 15
                  minutes.
                  <div style={{ marginLeft: 10, marginTop: 15 }}>
                    ...
                    <div style={{ marginTop: 10 }}>
                      <div style={{ display: "flex", alignItems: "baseline" }}>
                        ...
                        <div>
                          {" "}
                          If your on-chain tx is still pending, you may speed up
                          your transaction by increasing the gas price.{" "}
                        </div>
                      </div>
                    </div>
                    {item.isLocal && (
                      <div style={{ marginTop: 10 }}>
                        ...
                        >
                          ...
                          <div>
                            {" "}
                            If your on-chain tx has failed, this is usually
                            because the gas limit is set too low. You can
                            manually{" "}
                            <span
      ...
      case TransferHistoryStatus.TRANSFER_DELAYED:
        lab = (
          <Tooltip
            ...
            title={
              <span>
                Your fund is being processed on{" "}
                {item.dst_received_info.chain.name}, which usually takes 30-60
                minutes.
              </span>
            }
      ...
      case TransferHistoryStatus.TRANSFER_FAILED:
        lab = (
          <Tooltip
            ...
            title={
              <span>
                Your transaction has failed on {item.src_send_info.chain.name}.
                This is usually because the gas limit is set too low. Rest
                assured that your funds are safe. You may try again later.
              </span>
            }
            placement={isMobile ? "bottomLeft" : "right"}
            color="#fff"
          ...
          <Tooltip
            ...
            title={
              <span>
                ...
                {item.dst_received_info.chain.name}, which takes a few minutes
                in most cases but could take a few hours if there is heavy
                traffic or your transfer amount is large.
              </span>
            }
            placement={isMobile ? "bottomLeft" : "right"}
            color="#fff"
            ...
            title={
              <div>
                {peggedMode === PeggedChainMode.Deposit ? (
                  <span>
                    The transfer cannot be completed because there is not enough
                    liquidity to bridge your transfer. You may request a refund.
                  </span>
                ) : (
                  <span>
      ...
      case TransferHistoryStatus.TRANSFER_REQUESTING_REFUND:
        lab = (
          <Tooltip
            ...
            title={
              <span>
                Your refund request is being confirmed on Celer State Guardian
                Network (SGN), which may take a few minutes.
              </span>
            }
            placement={isMobile ? "bottomLeft" : "right"}
            color="#fff"
        ...
        lab = (
          <Tooltip
            ...
            title={
              showResult ? (
                <span>
                  Your request for refunding the transfer is being confirmed on{" "}
                  {item.src_send_info.chain.name}, which might take a few
                  minutes.
                </span>
              ) : (
                <div>
                    ...
                    <div style={{ marginTop: 10 }}>
                      <div style={{ display: "flex", alignItems: "baseline" }}>
                        ...
                        <div>
                          {" "}
                          If your on-chain tx has failed, this is usually
                          because the gas limit is set too low.Please{" "}
                          <span
                            style={{ color: "#1890ff", cursor: "pointer" }}
                            onClick={(e) => {
                              e.stopPropagation();
      ...
      case TransferHistoryStatus.TRANSFER_WAITING_FOR_SGN_CONFIRMATION:
        lab = (
          <Tooltip
            ...
            title={
              <span>
                Your transfer is being confirmed on Celer State Guardian Network
                (SGN), which might take a few minutes.
              </span>
            }
            placement={isMobile ? "bottomLeft" : "right"}
            color="#fff"
          ...
          <Tooltip
            ...
            title={
              <div style={{ display: "flex", alignItems: "center" }}>
                ...
                <div>
                  It seems that your tx was failed or stuck on-chain. If you
                  found the tx was failed, please increase your gas limit and
  ...
  if (historykey === "transfer_history") {
    historyContent = (
      <div
        className={
          themeType === "dark" ? classes.spinblur : classes.whiteSpinblur
        }
        key="1"
      >
            ...
            {mergedHistoryList.length > 0 ? (
              <div>
                {mergedHistoryList?.map((item) => {
                  ...
                  if (
                    item?.dst_received_info?.chain?.id === 56 &&
                    item?.src_send_info?.token.symbol === "BNB"
                  ) {
                    shouldDisplayMetaMaskIcon = false;
                  }

                  if (
                    item?.dst_received_info?.chain?.id === 560000 &&
                    item?.src_send_info?.token.symbol === "DFI"
                  ) {
                    shouldDisplayMetaMaskIcon = false;
                  }
                  ...
                  if (
                    item?.dst_received_info?.chain?.id === 250 &&
                    item?.src_send_info?.token.symbol === "FTM"
                  ) {
                    shouldDisplayMetaMaskIcon = false;
                  }

                  if (
                    item?.dst_received_info.chain.id === 336 &&
                    item?.src_send_info?.token.symbol === "SDN"
                  ) {
                    shouldDisplayMetaMaskIcon = false;
                  }

                  if (
                    item?.dst_received_info.chain.id === 137 &&
                    item?.src_send_info?.token.symbol === "MATIC"
                  ) {
                    shouldDisplayMetaMaskIcon = false;
                  }

                  if (
                    item?.dst_received_info.chain.id === 57 &&
                    item?.src_send_info?.token.symbol === "SYS"
                  ) {
                    shouldDisplayMetaMaskIcon = false;
                  }

                  if (
                    item?.dst_received_info.chain.id === 592 &&
                    item?.src_send_info?.token.symbol === "ASTR"
                  ) {
                    shouldDisplayMetaMaskIcon = false;
                  }

                  if (
                    item?.dst_received_info.chain.id === 16350 &&
                    item?.src_send_info?.token.symbol === "PEEL"
                  ) {
                    shouldDisplayMetaMaskIcon = false;
                  }

                  if (
                    item?.dst_received_info.chain.id === 13000 &&
                    item?.src_send_info?.token.symbol === "ECG"
                  ) {
                    shouldDisplayMetaMaskIcon = false;
                  }
                  ...
                  const srcChainEVMMode = getNonEVMMode(
                    item.src_send_info.chain.id
                  );
                  let srcBlockTxLink = item.src_block_tx_link;
                  if (
                    srcChainEVMMode === NonEVMMode.terraMainnet ||
                    srcChainEVMMode === NonEVMMode.terraTest
                  ) {
                    srcBlockTxLink = srcBlockTxLink.replace("0x", "");
                  }

                  const dstChainEVMMode = getNonEVMMode(
                    item.dst_received_info.chain.id
                  );
                  let dstBlockTxLink = item.dst_block_tx_link;
                  if (
                    dstChainEVMMode === NonEVMMode.terraMainnet ||
                    dstChainEVMMode === NonEVMMode.terraTest
                  ) {
                      ...
                      >
                        <div className={classes.itemLeft}>
                          <div
                            ...
                            style={
                              isMobile ? { minWidth: 0 } : { minWidth: 160 }
                            }
                          >
                            <div>
                              <img
                                src={item.src_send_info.chain.icon}
                                alt=""
                          ...
                          <div className={classes.itemtitle}>
                            <div>
                              <img
                                src={item?.dst_received_info.chain.icon}
                                alt=""
                                className={classes.txIcon}
                              />
                            </div>
                                ...
                                <div className={classes.receivetxnum}>
                                  ...
                                  {!isMobile && (
                                    <Tooltip
                                      overlayClassName={
                                        isMobile
                                          ? classes.mobileTooltipOverlayStyle
                                          : undefined
                                      }
                                    ...
                                    >
                                      ...
                                      <img
                                        ...
                                        height={14}
                                        style={{
                                          display: shouldDisplayMetaMaskIcon
                                            ? "flex"
                                            : "none",
                                          marginLeft: 5,
                                          cursor: "pointer",
                        ...
                        <div
                          className={
                            isMobile
                              ? classes.mobileItemRight
                              : classes.itemRight
                          }
                        >
                          <div
                            className={showSupport ? classes.showSuppord : ""}
                          >
                            <div>{tipsStatus(item, peggedMode)}</div>
                            <div className={classes.itemTime}>
                              {moment(Number(item.ts)).format(
                                "YYYY-MM-DD HH:mm:ss"
                              )}
                            </div>
                            {showSupport && (
          ...
          >
            <Menu.Item key="transfer_history">
              <div className={classes.tabtitle}>
                Transfer
                {historyActionNum !== 0 && (
                  <div className={classes.numdot}>{historyActionNum}</div>
                )}
              </div>
            </Menu.Item>
